...
{== LIST BOX ===================================================================}
procedure TFileListBox.SwapFileNames(index1,index2: integer);
  var t: string[12];
begin
  t:=filenames[index1];
...
begin
  numitems:=0; offset:=0;
  for i:=1 to FilesMax do filenames[i]:='';
...
begin
  ...
  if ItemsThatFit > numitems then itemsthatfit:=numitems;
...
begin
  if numitems<=itemsthatfit then begin
    exit;
  end;
  offset:=offset+OffD;
...
{  if offset>FilesMax-ItemsThatFit then begin offset:=FilesMax-ItemsThatFit; exit; end;}
  if offset+itemsthatfit>numitems then begin
    offset:=numitems-itemsthatfit;
    exit;
  end;
